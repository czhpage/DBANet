...
class DiffDW:
    ...
    def cal_weights(self, pred,  label):
        ...
        cur_cls_learn = torch.where(delta_dice>0, delta_dice, torch.tensor(0.0, dtype=delta_dice.dtype).cuda()) * torch.log(cur_dice / self.last_dice)
        cur_cls_unlearn = torch.where(delta_dice<=0, delta_dice, torch.tensor(0.0, dtype=delta_dice.dtype).cuda()) * torch.log(cur_dice / self.last_dice)
